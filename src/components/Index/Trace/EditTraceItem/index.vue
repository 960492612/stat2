...
<script>
import { editTraceItem } from "api/trace";
import {
  getChannelsByOwner,
...
export default {
  ...
  created() {
    if (this.$route.params.data) {
      ...
      this.data = {
        ...
        channel: params.subChannel_id,
      }
      this._getTopChannelsByOwner()
      this.changeTopChannel( params.topChannel_id)
      this.selectedTopChannel = params.topChannel_id
    }
  },
  computed: {
    ...mapGetters(["id"])
  ...
  methods: {
    ...
    _editTraceItem() {
      editTraceItem(this.data).then(res => {
        if (res.code == 1) {
          this.result = this.makeResult(this.$route.params.data.hash);
          this.$message({
            message: "修改成功",
            type: "success"
          });
